...
func CreateLinkToken(c *gin.Context, ps plaidinterface.Plaid, dbs postgresinterface.DBHandler, db *sql.DB, plaidapi *plaid.APIClient) {
	...
	request := ps.NewLinkTokenCreateRequest(user, strconv.Itoa(profile.ID), countryCodes, products, "")
...
func GetAccessToken(c *gin.Context, ps plaidinterface.Plaid, dbs postgresinterface.DBHandler, db *sql.DB, plaidapi *plaid.APIClient, debug bool) {
	ctx := context.Background()
